...
class Data_(object):
    def __init__(self,dataset_type):
        ...
        self.num_classes = 4
        self.anchors = [[[  9.4416859 ,  14.85094005],[ 22.28993193,  47.87918911],[ 33.25743693,  88.88096961]],[[ 71.33589601,  86.49159816],[ 48.77761854, 135.30687689],[ 71.51163254, 184.42727111]],[[154.88714247, 140.34443597],[105.34113661, 245.81126854],[181.67162257, 283.8130657 ],]]/np.array([[[8]],[[16]],[[32]]])#[[[10,14],[23,27],[37,58]],[[81,82],[135,169],[344,319]]]/np.array([[[8]],[[16]],[[32]]])
        self.anchor_per_scale = 3
        self.max_bbox_per_scale = 150

        self.csv_path = 'E:/chrome下载/2021-7-5/2train_rname3.csv'
        self.img_path_o = 'E:/chrome下载/2021-7-5/2_images.tar'

        self.samples = read_path(self.csv_path)
        self.num_samples = len(self.samples)
        self.num_batchs = int(np.ceil(self.num_samples / self.batch_size))
        self.batch_count = 0
        self.gt_bboxes,self.classes = load_gt_boxes(self.csv_path)

        self.class_dict = {'person':0 , 'badge':1 , 'clothes':2 , 'wrongclothes':3 }
        self.i = np.arange(0,self.num_samples)
        np.random.shuffle(self.i)

    def parse_annotations(self,i):
        img_path = self.img_path_o + '/' + self.samples[i]
        gt_bboxes,gt_classes = self.gt_bboxes[i] , self.classes[i]
        ...
        with tf.device('/cpu:0'):
            ...
            if self.batch_count < self.num_batchs:
                while num < self.batch_size:
                    ...
                    image , gt = self.parse_annotations(self.i[index])
                    bboxes , classes = gt
                ...
                return batch_image, (batch_smaller_target, batch_medium_target, batch_large_target)
            else:
                self.batch_count = 0
                np.random.shuffle(self.i)
                raise StopIteration
    ...
    def preprocess_true_boxes(self, bboxes,classes):
        ...
        label = [np.zeros((self.train_output_sizes[i], self.train_output_sizes[i], self.anchor_per_scale,
                           5 + self.num_classes)) for i in range(3)]#52,26,13
        ...
        for i in range(len(bboxes)):
            ...
            g = classes[i]
            # print(g)
            bbox_class_ind = self.class_dict[g]
