...
class RPNplus(tf.keras.Model):
    ...
    def call(self, x, training=False):
        ...
        pool3_p = self.region_proposal_conv1(pool3_p) # [1, 32, 32, 256]

        h = self.conv4_1(h)
        h = self.conv4_2(h)
        h = self.conv4_3(h)
        h = self.pool4(h)
        pool4_p = self.region_proposal_conv2(h) # [1, 32, 32, 512]

        h = self.conv5_1(h)
        h = self.conv5_2(h)
        h = self.conv5_3(h)
        pool5_p = self.region_proposal_conv3(h) # [1, 32, 32, 512]

        region_proposal = tf.concat([pool3_p, pool4_p, pool5_p], axis=-1) # [1, 32, 32, 1280]

        conv_cls_scores = self.scores_conv(region_proposal) # [1, 32, 32, 18]
        conv_cls_bboxes = self.bboxes_conv(region_proposal) # [1, 32, 32, 36]

        cls_scores = tf.reshape(conv_cls_scores, [-1, 32, 32, 9, 2])
        cls_bboxes = tf.reshape(conv_cls_bboxes, [-1, 32, 32, 9, 4])
